"""MEXC Funding Provider adapter (ADR 017 / RFC 005).

Guides USDT withdrawals to a RoamKit ``WalletAddress``. Never mutates Credits.
Provider history (e.g. hisrec) is not Credits SoT — Observation + Confirmation is.
"""
...
# MEXC UI network label for Polygon PoS (ops catalog; not Wallet domain).
_DEFAULT_NETWORK_LABEL = "MATIC"


class MexcFundingProvider:
    """MEXC adapter: present destination + withdraw guide / status for UX."""

    def metadata(self) -> FundingProviderMetadata:
        label = getattr(settings, "MEXC_POLYGON_NETWORK_LABEL", "") or ""
        ...
        return FundingProviderMetadata(
            ...
            extra={
                "withdraw_url": getattr(settings, "MEXC_WITHDRAW_URL", "") or "",
            },
        )
    ...
    def deposit(self, request: FundingDepositRequest) -> FundingDepositGuide:
        ...
        instructions = (
            f"On MEXC, withdraw {_ASSET_USDT} via network {network} to "
            f"{destination}. Do not use other networks. Credits appear only after "
            f"RoamKit confirms the on-chain transfer (not when MEXC marks withdraw "
            f"complete)."
        )
    ...
    def status(self, session_id: str) -> FundingStatus:
        ...
        return FundingStatus(
            ...
            message=(
                ...
                "MEXC withdraw status is UX-only; Credits require on-chain "
                "Confirmation."
            ),
    ...
    def _validate_request(self, request: FundingDepositRequest) -> None:
        ...
        if asset != _ASSET_USDT:
            raise FundingProviderUnsupportedError(
                f"MEXC adapter supports {_ASSET_USDT} only, got {request.asset!r}"
            )
        if request.chain != WalletChain.POLYGON:
            raise FundingProviderUnsupportedError(
                f"MEXC adapter supports polygon only, got {request.chain!r}"
            )
    ...
    def _session_id(*, wallet_address_id: str, chain: str, asset: str) -> str:
        return f"mexc:v1:{wallet_address_id}:{chain}:{asset}"

    @staticmethod
    def _parse_session_id(session_id: str) -> tuple[str, str, str]:
        parts = (session_id or "").split(":")
        if len(parts) != 5 or parts[0] != "mexc" or parts[1] != "v1":
            raise FundingProviderError(f"invalid MEXC session_id: {session_id!r}")
