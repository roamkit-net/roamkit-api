...
class PolygonProvider:
    ...
    def fetch_usdt_transfer(self, tx_hash: str) -> TransferResult:
        """Return the USDT transfer to the platform wallet for ``tx_hash``.
        ...
        ``POLYGON_PLATFORM_WALLET``. Other tokens and other recipients are
        skipped. Confirmations are reported only; min-confirm gating belongs to
        ...
        matched = self._find_usdt_transfer_log(receipt.get("logs") or [])
        if matched is None:
            raise TransferNotFoundError(
                f"No USDT transfer to platform wallet in {normalized_hash}"
            )

        from_address, to_address, amount, log = matched
    ...
    def _find_usdt_transfer_log(
        ...
        """Return the first USDT→platform Transfer in receipt log order."""
        for log in logs:
            ...
            if to_address != self.platform_wallet:
                continue
